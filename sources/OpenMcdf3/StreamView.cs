...
namespace OpenMcdf3
{
    ...
    internal sealed class StreamView : Stream
    {
        ...
        {
            if (sectorChain == null)
                throw new ArgumentNullException("Sector Chain cannot be null", nameof(sectorChain));

            if (sectorSize <= 0)
                throw new ArgumentException("Sector size must be greater than zero", nameof(sectorSize));
        ...
        public StreamView(List<Sector> sectorChain, int sectorSize, long length, Queue<Sector> availableSectors, bool isFatStream = false)
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            // Ensure read request greater then stream length, when position is not 0,
            // return only the limited and correct number of bytes
            ...
            if (BaseSectorChain != null && BaseSectorChain.Count > 0)
            {
                ...
                if (nToRead != 0)
                {
                    if (secIndex > BaseSectorChain.Count) throw new FileFormatException("The file is possibly corrupted.");
        ...
        private void AdjustLength(long value, Queue<Sector> availableSectors)
        {
            ...
            if (delta > 0)
            {
                ...
                while (nSec > 0)
                {
                    ...
                    {
                        t = new Sector(sectorSize, SectorType.Normal);
            ...
            if (BaseSectorChain != null)
            {
                ...
                if (secOffset < BaseSectorChain.Count)
                {
                    Buffer.BlockCopy(
                        buffer,
                        offset,
                        BaseSectorChain[secOffset].Data,
                        secShift,
                        roundByteWritten);
